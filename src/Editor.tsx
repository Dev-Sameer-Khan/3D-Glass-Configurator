...
import { Environment, ContactShadows, OrbitControls, Grid, Stats } from '@react-three/drei';
...
export default function Editor() {
  ...
  const estimate = useMemo(() => {
    ...
    const styleMultiplier = product === 'patch-fitting-door' ? STYLE_MULTIPLIER.patch : STYLE_MULTIPLIER[style];
    const glassCost = areaSqFt * baseRate * styleMultiplier * PRODUCT_MULTIPLIER[product];
    ...
    <div className="w-screen h-screen bg-[#222] text-slate-800 overflow-hidden relative flex flex-col md:flex-row font-sans select-none">
      ...
      >
        ...
        >
          ...
          <OrbitControls target={[0, height / 2, 0]} minPolarAngle={Math.PI / 6} maxPolarAngle={Math.PI / 2 + 0.1} />
          <Grid
            cellSize={0.5}
            cellThickness={0.5}
            cellColor="#3b82f6"
      ...
      >
        <div className="flex items-center gap-3 mb-6 pb-4 border-b border-slate-200">
          ...
          <div>
            <h1 className="text-sm font-semibold uppercase tracking-tight text-slate-900">3D Glass Configurator</h1>
            <p className="text-[10px] uppercase font-mono tracking-widest text-slate-500">Modern Industrial</p>
          </div>
        </div>
        <div className="space-y-6 flex-1">
          ...
          <div>
            <label className="flex items-center gap-2 text-[11px] uppercase text-slate-500 font-semibold mb-3">
              <Sparkles size={14} className="text-blue-900" /> {product + " Style"}
            </label>
            <div className="grid grid-cols-2 gap-2">
              <button
                ...
                className={`py-2 px-3 rounded text-center text-[10px] font-bold uppercase transition-all ${
                  style === 'patch' ? 'bg-blue-100 border border-blue-900/40 text-blue-900' : 'bg-slate-50 border border-slate-200 text-slate-600 hover:text-slate-900'
              ...
              <button
                ...
                className={`py-2 px-3 rounded text-center text-[10px] font-bold uppercase transition-all ${
                  style === 'frame' ? 'bg-blue-100 border border-blue-900/40 text-blue-900' : 'bg-slate-50 border border-slate-200 text-slate-600 hover:text-slate-900'
                } ${product === 'patch-fitting-door' ? 'opacity-50 cursor-not-allowed' : ''}`}
              >
                Full Frame
              </button>
          ...
          <div>
            ...
            <div className="relative">
              <select
                value={glassType}
                onChange={(e) => setGlassType(e.target.value as 'clear' | 'frosted' | 'tinted')}
                className="w-full bg-slate-50 border border-slate-200 text-slate-800 text-[11px] rounded p-2.5 outline-none appearance-none cursor-pointer"
              >
          ...
          <div className="pt-6 border-t border-slate-200 mt-2 space-y-2">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setIsOpen(!isOpen)} className={`w-10 h-5 rounded-full relative transition-colors ${isOpen ? 'bg-blue-900' : 'bg-slate-300'}`}>
            ...
            <div className="rounded-lg bg-slate-50 border border-slate-200 p-3 text-xs space-y-1">
              <div className="flex justify-between"><span>Area</span><span>{estimate.areaSqFt.toFixed(2)} sq.ft</span></div>
              <div className="flex justify-between"><span>Rate</span><span>Rs {estimate.baseRate}/sq.ft</span></div>
              <div className="flex justify-between"><span>Subtotal</span><span>Rs {estimate.subTotal.toFixed(0)}</span></div>
              <div className="flex justify-between font-semibold"><span>Total</span><span>Rs {estimate.total.toFixed(0)}</span></div>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="absolute inset-0 z-50 bg-black/40 p-3 md:p-8 flex items-center justify-center">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-3xl max-h-[92vh] overflow-y-auto border border-slate-200">
            <div className="sticky top-0 bg-white border-b border-slate-200 p-4 flex items-center justify-between">
              <h3 className="font-semibold text-slate-900">Quotation Preview</h3>
              <button
            ...
            <div className="p-5 md:p-7 space-y-6">
              <div className="flex flex-col md:flex-row md:justify-between gap-4">
                <div>
                  <h2 className="text-xl font-semibold text-blue-900">Axis Glass & Hardware</h2>
                  <p className="text-sm text-slate-600">Professional 3D Configuration Quote</p>
                </div>
              ...
              <div className="grid grid-cols-2 gap-3 text-sm bg-slate-50 border border-slate-200 rounded-lg p-4">
                <p><span className="font-semibold">Product:</span> {PRODUCT_LABEL[product]}</p>
                <p><span className="font-semibold">Style:</span> {style === 'patch' ? 'Patch Fitting' : 'Full Frame'}</p>
                <p><span className="font-semibold">Width:</span> {widthMm} mm</p>
                <p><span className="font-semibold">Height:</span> {heightMm} mm</p>
                <p><span className="font-semibold">Glass:</span> {glassType}</p>
              </div>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    <tr className="border-t border-slate-200">
                      ...
                      <td className="px-4 py-2 text-right">Rs {estimate.glassCost.toFixed(2)}</td>
                    </tr>
                    <tr className="border-t border-slate-200">
                      <td className="px-4 py-2">Labor</td>
                      <td className="px-4 py-2 text-right">Rs {LABOR_CHARGE.toFixed(2)}</td>
                    </tr>
                    <tr className="border-t border-slate-200">
                      <td className="px-4 py-2">Tax ({TAX_PERCENT}%)</td>
                      <td className="px-4 py-2 text-right">Rs {estimate.tax.toFixed(2)}</td>
                    </tr>
                    <tr className="border-t border-slate-200 bg-slate-900 text-white font-semibold">
                      <td className="px-4 py-2">Grand Total</td>
                      <td className="px-4 py-2 text-right">Rs {estimate.total.toFixed(2)}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
              <div className="flex justify-end">
                <button
                  onClick={() => window.alert('PDF export can be wired next with jsPDF.')}
